...
* `parseAd(adElement)` becomes `parseAd(adElement, emit)`. The param `emit` is a function used to trigger warning events logs
* `parseInLine(inLineElement)` becomes `parseInLine(inLineElement, emit)` to trigger warnings.
* `parseWrapper(wrapperElement)` becomes `parseWrapper(wrapperElement, emit)` to trigger warnings.

Warning events are actually triggered when the required VAST values are missing according to IAB specifications. It carries the following data:

- `message: String` - Message describing missing required value
- `parentElement: String` - Parent element of missing required value
- `specVersion: Number` - IAB specification version (current one: 4.1)

For more explainations, please consult warning event section in [vast-parser.md](vast-parser.md#events)

## Version 3.0.0-alpha.8 (current version)
